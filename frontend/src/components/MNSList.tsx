...
import { Accordion, Button, Input, PopupModal, PopupModalContent, PopupModalHeader, Spinner, Tooltip } from '@massalabs/react-ui-kit';
...
export function MNSList() {
  ...
  const { getUserEntryList, deleteDnsEntry, changeTargetAddressDnsEntry } = useWriteMNS(massaClient);
  const [spinning, setSpinning] = useState(false);
  const [list, setList] = useState<DnsUserEntryListResult[]>([]);

  const [changeTargetModalId, setChangeTargetModalId] = useState<string | null>(null);
              ...
              <div className="fixed inset-0 flex items-center justify-center">
                <PopupModal onClose={() => setChangeTargetModalId(null)}>
                  <PopupModalHeader customClassHeader="mb-8">
                    <h2 className="mas-h2">Change target address of {changeTargetModalId}.massa</h2>
                  </PopupModalHeader>
                  <PopupModalContent>
                    ...
                    <div className="flex flex-row-reverse pb-12">
                      <div className="w-32">
                        <Button onClick={() => {
                          changeTargetAddressDnsEntry({domain: changeTargetModalId, targetAddress: newTargetAddress});
                ...
                <div key={idx} className="bg-secondary rounded-xl p-4 mb-4">
                  <div className="flex flex-row">
                    <div className="flex justify-between grow">
                      <p className="mas-body my-auto">{item.domain + '.massa'}</p>
                      <ChevronDoubleRightIcon className="w-6" />
                      <Tooltip body={item.targetAddress}>
                        <p className="mas-body pr-4 my-auto	">{item.targetAddress.slice(0, 10)}...</p>
                      </Tooltip>
                    </div>
                    <button className="flex" onClick={() => { setChangeTargetModalId(item.domain) }}>
                      <PencilIcon className="w-5 mr-2 my-auto" />
                      <p className="mas-body mr-4 my-auto">Change target</p>
                    </button>
                    <button className="flex" onClick={() => { deleteDnsEntry({ tokenId: item.tokenId }) }}>
