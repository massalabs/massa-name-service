...
export function constructor(_: StaticArray<u8>): void {
  ...
  const buildnetContract =
    'AS12qKAVjU1nr66JSkQ6N4Lqu4iwuVc6rAbRTrxFoynPrPdP1sj3G';
  const keys = getKeysOf(buildnetContract);
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    const value = Storage.getOf(new Address(buildnetContract), key);
    Storage.set(key, value);
  }
  generateEvent('Buildnet contract cloned ');

  // const NB_DOMAINS = 3000;
  // _mintBatch(NB_DOMAINS);
}

export function mintBatch(_: StaticArray<u8>): void {
  _onlyOwner();
  const NB_DOMAINS = 2000;
  _mintBatch(NB_DOMAINS);
}

export function mintBatchMixedTarget(_: StaticArray<u8>): void {
  _onlyOwner();

  const NB_DOMAINS = 3000;
  _mintBatchMixedTarget(NB_DOMAINS);
}

function _mintBatchMixedTarget(batchSize: i32): void {
  const owner = Context.caller().toString();

  let counter = bytesToU256(Storage.get(COUNTER_KEY));
  let domainCounterI32 = counter.toI32();
  let lastTokenId: u256 = counter;

  generateEvent('Minting batch from id ' + counter.toI32().toString());

  const batchEnd = domainCounterI32 + batchSize;
  const batchStart = counter.toI32() + 1;

  for (
    domainCounterI32 = batchStart;
    domainCounterI32 <= batchEnd;
    domainCounterI32++
  ) {
    const domain = 'mixed' + domainCounterI32.toString();
    const target = 'target' + domainCounterI32.toString();

    const addressKey = buildAddressKey(target);

    lastTokenId = u256.fromI32(domainCounterI32);
    // Transfer ownership of the domain to the caller
    _update(owner, lastTokenId, '');

    const domainBytes = stringToBytes(domain);
    Storage.set(buildDomainKey(lastTokenId), domainBytes);
    Storage.set(buildTokenIdKey(domain), u256ToBytes(lastTokenId));

    Storage.set(addressKey, domainBytes);
  }
  generateEvent('Minted to tokenId ' + (domainCounterI32 - 1).toString());
  Storage.set(COUNTER_KEY, u256ToBytes(lastTokenId));
}

function _mintBatch(batchSize: i32): void {
  const owner = Context.caller().toString();

  const addressKey = buildAddressKey(TARGET);
  let counter = bytesToU256(Storage.get(COUNTER_KEY));
  let domains: StaticArray<u8> = [];
  let coma: StaticArray<u8> = [44];
  let domainCounterI32 = 0;

  if (Storage.has(addressKey)) {
    domains = Storage.get(addressKey);
  }

  const batchStart = counter.toI32() + 1;
  const batchEnd = batchStart + batchSize;

  generateEvent('Minting batch from id ' + batchStart.toString());
  ...
  for (
    ...
    const domain = 'y'.repeat(15) + domainCounterI32.toString();
    // const domain = DOMAINS[domainCounterI32 % DOMAINS.length];
    ...
    // add a coma, except for the first domain
    if (domainCounterI32 != batchStart) {
      domains = domains.concat(coma);
    }

    domains = domains.concat(domainBytes);
  }
  generateEvent('Minted to tokenId ' + (domainCounterI32 - 1).toString());
  Storage.set(addressKey, domains);
  Storage.set(COUNTER_KEY, u256ToBytes(lastTokenId));
}
...
export function migrate(binaryArgs: StaticArray<u8>): void {
  ...
  let migrateCounter = bytesToI32(Storage.get(migrateCounterdKey()));

  let keyIndex = migrateCounter;
  ...
  while (keyIndex < migrateCounter + BATCH_SIZE) {
    ...
    const domainByteLen = domainsBytes.length;

    const targetBytes = StaticArray.fromArray(
      key.slice(targetToDomainsPrefix.length),
    );

    let domainStart = 0;
    for (let idx = 0; idx < domainByteLen; idx++) {
      let lastDomainOffset = 0;
      if (idx == domainByteLen - 1) {
      ...
      const newKey = targetToDomainKey(targetBytes, domainBytes);
  ...
  Storage.set(migrateCounterdKey(), i32ToBytes(keyIndex));
  generateEvent(
    'done. processed keys:' + (keyIndex - migrateCounter).toString(),
  );
}

export function initMig(_binaryArgs: StaticArray<u8>): void {
  _onlyOwner();
  Storage.set(migrateCounterdKey(), i32ToBytes(0));
}
