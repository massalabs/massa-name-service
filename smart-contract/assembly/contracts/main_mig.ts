...
export function clean(_binaryArgs: StaticArray<u8>): void {
  _onlyOwner();
  ...
  for (let i = 0; i < targetToDomainsKeys.length; i++) {
    const key = targetToDomainsKeys[i];
    if (!Storage.has(key)) {
  ...
  Storage.del(migrateCounterdKey());
  const refund = balance() - initBal;
  if (refund > 0) {
    transferCoins(Context.caller(), refund);
    generateEvent('Refunded MAS: ' + refund.toString());
  }
}
