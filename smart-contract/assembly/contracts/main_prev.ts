...
function _mintBatch(batchSize: i32): void {
  ...
  for (
    ...
    const domain = 'domain' + domainCounterI32.toString();
...
export function migrate(binaryArgs: StaticArray<u8>): void {
  ...
  while (processedKeys < BATCH_SIZE) {
    ...
    if (nbDomainsTargeted > 1) {
      generateEvent(
        'big boy!!! nbDomainsTargeted: ' + nbDomainsTargeted.toString(),
      );
    }
